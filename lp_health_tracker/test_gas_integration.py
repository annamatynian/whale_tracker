...
async def test_gas_integration():
    ...
    try:
        ...
        print("✅ LPHealthTracker initialized")
        
        # Check if gas_calculator is properly initialized as None
        if tracker.gas_calculator is None:
            print("✅ GasCostCalculator properly initialized as None before connections")
        else:
            print("❌ GasCostCalculator should be None before initialize_connections()")
            return False
        ...
        try:
            ...
            if success:
                ...
                if tracker.gas_calculator is not None:
                    print("✅ GasCostCalculator properly initialized after connections")
                    print(f"✅ GasCostCalculator type: {type(tracker.gas_calculator)}")
                else:
                    print("❌ GasCostCalculator should be initialized after initialize_connections()")
                    return False
            else:
                print("⚠️  Connection initialization failed (expected in test environment)")
...
async def test_gas_methods():
    ...
    try:
        ...
        if hasattr(tracker, 'update_all_gas_costs'):
            print("✅ update_all_gas_costs method exists")
        else:
            print("❌ update_all_gas_costs method missing")
            return False
